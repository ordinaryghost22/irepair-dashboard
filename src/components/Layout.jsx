  ...
  return (
    <div style={{ display:"flex", height:"100vh", background:t.pageBg, fontFamily:"'Inter',system-ui,sans-serif", transition:"background .25s" }}>
      ...
      {mobileOpen && (
        <div onClick={() => setMobileOpen(false)}
          ...
          <div onClick={e => e.stopPropagation()}
            style={{ width:260, height:"100%", background:"linear-gradient(180deg,#080b14 0%,#0f1120 55%,#0d1028 100%)", animation:"slideIn .22s ease", display:"flex", flexDirection:"column" }}>
            <SidebarContent
              location={location} navigate={navigate}
              pendingCount={pendingCount} clearBadge={clearBadge}
      ...
      {!isMobile && sidebarOpen && (
        <aside style={{ width:240, flexShrink:0, background:"linear-gradient(180deg,#080b14 0%,#0f1120 55%,#0d1028 100%)", display:"flex", flexDirection:"column", borderRight:"1px solid rgba(255,255,255,0.06)", zIndex:10 }}>
          <SidebarContent
            location={location} navigate={navigate}
            pendingCount={pendingCount} clearBadge={clearBadge}
            dark={dark} toggle={toggle}
      ...
      <div style={{ flex:1, display:"flex", flexDirection:"column", overflow:"hidden", minWidth:0 }}>
        {/* Topbar */}
        <div style={{ height:58, background:t.topbarBg, borderBottom:`1px solid ${t.border}`, display:"flex", alignItems:"center", padding:`0 ${isMobile?14:20}px`, gap:isMobile?8:12, flexShrink:0 }}>
          <button onClick={toggleMenu}
            style={{ background:"none", border:"none", cursor:"pointer", fontSize:20, color:t.textMuted, padding:"4px 6px", borderRadius:8, flexShrink:0 }}>
            ☰
          </button>

          {/* Search — hidden on mobile to save space, icon opens it */}
          {!isMobile && <GlobalSearch />}

          <div style={{ flex:1 }} />

          {/* Live dot + time — compact on mobile */}
          <span style={{ fontSize:11, color:t.textMuted, display:"flex", alignItems:"center", gap:5, flexShrink:0 }}>
            <span style={{ width:7, height:7, borderRadius:"50%", background:isPaused?"#9ca3af":"#22c55e", display:"inline-block", boxShadow:isPaused?"none":"0 0 6px rgba(34,197,94,0.6)" }} />
            {!isMobile && lastFetchStr}
          </span>

          <NotifBell />

          <button onClick={() => setIsPaused(!isPaused)}
            style={{ display:"flex", alignItems:"center", gap:6, padding:isMobile?"7px 10px":"7px 14px", borderRadius:20, border:`1px solid ${t.border}`, background:t.cardBg2, color:isPaused?t.textMuted:"#22c55e", fontSize:12, fontWeight:600, cursor:"pointer", flexShrink:0 }}>
            {isPaused ? "⏸" : "🔄"}{!isMobile && (isPaused?" Paused":" Live")}
          </button>

          {newBadge > 0 && (
            <div onClick={() => clearBadge()}
              style={{ background:"linear-gradient(135deg,#ef4444,#dc2626)", color:"#fff", fontSize:11, fontWeight:700, padding:"6px 10px", borderRadius:20, animation:"badgePulse 1.5s infinite", cursor:"pointer", flexShrink:0 }}>
              🔔 {newBadge}
            </div>
          )}
        </div>
        ...
        {isMobile && (
          <div style={{ padding:"10px 14px", background:t.topbarBg, borderBottom:`1px solid ${t.border}` }}>
            <GlobalSearch />
          </div>
        )}
        ...
        {NAV_ITEMS.map(({ path, label, icon, badge }) => {
          ...
          return (
            <Link key={path} to={path} onClick={() => { badge && clearBadge(); onNav && onNav(); }}>
              <div
                style={{ display:"flex", alignItems:"center", gap:11, padding:"11px 13px", borderRadius:11, marginBottom:3, cursor:"pointer", transition:"all .15s",
...
function NavBtn({ icon, label, onClick, color="rgba(255,255,255,0.4)", hoverColor="rgba(255,255,255,0.75)", hoverBg="rgba(255,255,255,0.06)" }) {
  return (
    <div onClick={onClick}
      style={{ display:"flex", alignItems:"center", gap:11, padding:"11px 13px", borderRadius:11, color, fontSize:14, cursor:"pointer", transition:"all .15s", marginBottom:2 }}
