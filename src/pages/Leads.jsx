    ...
    <div style={{ padding: "20px 16px", maxWidth: 1400, animation: "fadeIn .3s ease" }}>
      ...
      <div className="leads-header" style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20 }}>
        ...
        <button
          ...
          style={{ ...secondaryBtnStyle(t), padding: "9px 18px", fontSize: 13, whiteSpace: "nowrap" }}
        >
          Export CSV
        </button>
      </div>
      <div style={{ position: "relative", marginBottom: 14 }}>
        <span style={{ position: "absolute", left: 14, top: "50%", transform: "translateY(-50%)", fontSize: 15, color: t.textMuted }}>
          🔍
        </span>
        <input
          value={search}
      ...
      {filtered.length === 0 ? (
        <EmptyState icon="👥" title="No leads found" subtitle="Leads from your booking bot will appear here" />
      ) : (
        <div style={{ background: t.cardBg, borderRadius: 18, border: `1px solid ${t.border}`, boxShadow: t.cardShadow, overflow: "hidden" }}>
          <div style={{ overflowX: "auto" }}>
            <table className="leads-table" style={{ width: "100%", borderCollapse: "collapse", minWidth: 320 }}>
              <thead>
                <tr>
                  {["Name", "Phone", "Device", "Issue", "First Contact", ""].map((h) => (
                    <th key={h || "actions"} style={TH}>
                      {h}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {filtered.map((l, i) => (
                  ...
                  >
                    ...
                    <td style={{ ...TD, width: 48, textAlign: "right" }}>
                      {l.id ? (
                        <button
                          type="button"
                          className="leads-del-btn"
                          title="Delete lead"
