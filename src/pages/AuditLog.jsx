  ...
  return (
    <div style={{ padding:isMobile?14:32, maxWidth:1000, animation:"fadeIn .3s ease" }}>
      ...
      <div style={{ marginBottom:20 }}>
        <h1 style={{ fontSize:isMobile?20:22, fontWeight:800, color:t.textPrimary, letterSpacing:-0.6, margin:0 }}>Audit Log</h1>
        <p style={{ color:t.textSecondary, fontSize:13, marginTop:4 }}>Every action taken on bookings</p>
      </div>
              ...
              <tbody>
                {auditLog.map((e,i)=>(
                  <tr key={i} style={{transition:"background .12s"}} onMouseEnter={x=>x.currentTarget.style.background=t.rowHover} onMouseLeave={x=>x.currentTarget.style.background="transparent"}>
                    ...
                    <td style={{...TD,fontFamily:"monospace",fontSize:10,color:t.textMuted}}>{isMobile?(e.bookingId||"—").slice(0,10)+"…":e.bookingId}</td>
