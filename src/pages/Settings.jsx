...
export default function Settings() {
  ...
  const section = { background:t.cardBg, borderRadius:18, border:`1px solid ${t.border}`, boxShadow:t.cardShadow, padding:28, marginBottom:20 };
  ...
  const field = (label, defaultValue, type="text") => (
    <div key={label}>
      ...
      <input type={type} defaultValue={defaultValue}
        style={{ width:"100%", padding:"12px 16px", borderRadius:12, background:t.inputBg, border:`1px solid ${t.border}`, color:t.textPrimary, fontSize:14, outline:"none", transition:"border .2s" }}
        onFocus={e=>e.target.style.border=`1px solid ${t.accent}`} onBlur={e=>e.target.style.border=`1px solid ${t.border}`} />
    </div>
  );

  return (
    <div style={{ padding:32, maxWidth:720, animation:"fadeIn .3s ease" }}>
      ...
      <div style={section}>
        ...
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"14px 18px", background:t.cardBg2, borderRadius:14, border:`1px solid ${t.border}` }}>
          <div>
      ...
      <button onClick={()=>{setSaved(true);setTimeout(()=>setSaved(false),2500);}} style={{ padding:"13px 32px", borderRadius:14, border:"none", background:saved?"linear-gradient(135deg,#22c55e,#16a34a)":"linear-gradient(135deg,#667eea,#764ba2)", color:"#fff", fontWeight:700, fontSize:14, cursor:"pointer", boxShadow:saved?"0 6px 24px rgba(34,197,94,.35)":"0 6px 24px rgba(102,126,234,.35)", transition:"all .3s" }}>
