...
export default function Settings() {
  ...
  const section = { background:t.cardBg, borderRadius:18, border:`1px solid ${t.border}`, boxShadow:t.cardShadow, padding:isMobile?18:28, marginBottom:16 };
  const sTitle  = { fontWeight:700, fontSize:14, color:t.textPrimary, marginBottom:16, paddingBottom:12, borderBottom:`1px solid ${t.borderSub}` };
  ...
  const field = (label, defaultValue, type="text") => (
    <div key={label}>
      ...
      <input type={type} defaultValue={defaultValue}
        style={{ width:"100%", padding:"12px 16px", borderRadius:12, background:t.inputBg, border:`1px solid ${t.border}`, color:t.textPrimary, fontSize:14, outline:"none" }}
        onFocus={e=>e.target.style.border=`1px solid ${t.accent}`} onBlur={e=>e.target.style.border=`1px solid ${t.border}`} />
    </div>
  );

  return (
    <div style={{ padding:p, maxWidth:720, animation:"fadeIn .3s ease" }}>
      <style>{"@keyframes fadeIn{from{opacity:0;transform:translateY(8px)}to{opacity:1;transform:translateY(0)}}"}</style>
      <div style={{ marginBottom:24 }}>
        <h1 style={{ fontSize:isMobile?20:22, fontWeight:800, color:t.textPrimary, letterSpacing:-0.6, margin:0 }}>Settings</h1>
        <p style={{ color:t.textSecondary, fontSize:13, marginTop:4 }}>Manage your shop details and preferences</p>
      </div>
      ...
      <div style={section}>
        ...
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"14px 16px", background:t.cardBg2, borderRadius:14, border:`1px solid ${t.border}` }}>
          ...
          <div onClick={toggle} style={{ width:52, height:28, borderRadius:14, background:dark?"linear-gradient(135deg,#667eea,#764ba2)":"#e2e8f0", cursor:"pointer", position:"relative", transition:"all .25s", flexShrink:0, marginLeft:12 }}>
      ...
      {/* Shop Info — 1 col on mobile, 2 on desktop */}
      <div style={section}>
        <div style={sTitle}>🏪 Shop Information</div>
        <div style={{ display:"grid", gridTemplateColumns:cols, gap:isMobile?14:18 }}>
      ...
      <div style={section}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:cols, gap:isMobile?14:16 }}>
      ...
      <button onClick={()=>{setSaved(true);setTimeout(()=>setSaved(false),2500);}}
        style={{ width:isMobile?"100%":"auto", padding:"13px 32px", borderRadius:14, border:"none", background:saved?"linear-gradient(135deg,#22c55e,#16a34a)":"linear-gradient(135deg,#667eea,#764ba2)", color:"#fff", fontWeight:700, fontSize:14, cursor:"pointer", boxShadow:saved?"0 6px 24px rgba(34,197,94,.35)":"0 6px 24px rgba(102,126,234,.35)", transition:"all .3s" }}>
